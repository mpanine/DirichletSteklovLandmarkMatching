function [ to_keep ] = random_exclude_4neighs( W, landmark_indices, NUM_LMKS )
    ...
    for idx=1:NUM_LMKS
        ...
        % 4-ring neighs of indices
        neighs = find(W(landmark_index_random,:));% 1-ring
        landmark_indices = setdiff(landmark_indices,neighs);
        neighs2 = find(sum(W(neighs,:),1));% 2-ring
        landmark_indices = setdiff(landmark_indices,neighs2);
        neighs2 = find(sum(W(neighs2,:),1));% 3-ring
        landmark_indices = setdiff(landmark_indices,neighs2);
        neighs2 = find(sum(W(neighs2,:),1));% 4-ring
