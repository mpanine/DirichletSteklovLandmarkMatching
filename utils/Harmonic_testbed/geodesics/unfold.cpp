...
protected:
	...
	void MarchAlg(struct Vertex  *temp_V,	
				  int temp_Vnum,
				  double *srcval);
	void __forceinline __fastcall CallTUpdateNeighbors(int             i,	/* vertex number */
...
/***************************************************************************/
/* L2  error the diff between the distance _and_ U*/
/***************************************************************************/
...
/***************************************************************************/
/* L1  error norm the diff between the distance _and_ U*/
/***************************************************************************/
...
/***************************************************************************/
/* CosAngle the cos of the angle at the vertex v0, between v1 _and_ v2	   */
/***************************************************************************/
double CosAngle(int v0,int v1,int v2,struct Vertex *V)
{
	double x1,x2,y1,y2,z1,z2,res;
	if(v0 != -1 _and_ v1 != -1 _and_ v2 != -1){
		x1 = V[v1].x - V[v0].x;
		x2 = V[v2].x - V[v0].x;
		y1 = V[v1].y - V[v0].y;
		y2 = V[v2].y - V[v0].y;
...
/***************************************************************************/
/* Length between the vertex v0 _and_ v1									   */
/***************************************************************************/
double
...
/***************************************************************************/
/* nextT next triangle to be unfolded. find the triangle #, _and_ the vertex */
/* Returns true is the next triangle was found.							   */
/* v1 _and_ v2 indicate the edge that is common to the original triangle	   */
/* _and_ the triangle to be unfolded.										   */
/* ti is the original triangle _and_ v3 is the other vertex of the triangle  */
/* to be unfolded.														   */
      ...
      struct Triangle * T, struct Vertex * V, int Tnum)
{
	...
	for (i = 0; i < V[v1].ti _and_ _not_ found; i++) {
		tj = V[v1].Tind[i];
		if (tj < Tnum _and_ tj != ti)
			/* search for tj in the list of v2 */
			for (k = 0; k < V[v2].ti _and_ _not_ found; k++) 
...
/***************************************************************************/
/* Split obtuse angles by unfolding splitting _and_ connecting, return the   */
/* number of unfoldings that were nessesery.							   */
/* ti is the tirnalge to be splittined, V0 is the vertex with the obtuse   */
/* angle while V1 _and_ V2 are the other vertexes of ti.					   */
/***************************************************************************/
int
      ...
      int NonSplitTnum, int Vnum)
{
	double          xv1,x1, y1, yv1,
					x2,				/* The distance between V0 _and_ V2 */
					y2,
					x3,y3, xt2, xt3, yt3,
					e0,				/* The distance between v1 _and_ v2 */
					e1,				/* The distance between v2 _and_ v3.*/
					e2,				/* The distance between v0 _and_ v1 */
					ta,				/* Tan of alpha.				  */
					cb, sb;			/* Cos _and_ Sin of beta.			  */
	int             v1 = V1, v2 = V2, v3,
	...
	/* if there is a next triangle _and_ _not_ splited */
	while (nextT(tm, v1, v2, &v3, &tn, T, V, NonSplitTnum) _and_ (_not_ splitter) ) {
		count++;
		...
		if (x3 > 0 _and_ y3/x3 > ta) {		/* if we found a splitter */
			splitter = TRUE;
		...
		else {						   /* we have _not_ found a splitter,
										  continue unfolding			*/
			if (x3 < 0){
				v1 = v3; x1 = x3; y1 = y3;
			} else {
	...
	for (i = 0; i < Vnum; i++) {		/* scan all vertices */
		for (ti = 0; ti < V[i].ti; ti++){/* scan connected triangles */
			if (V[i].Tind[ti] < NonSplitTnum) {	/* if valid triangle */
												/* Make v1 _and_ v2 the neighbours.			*/
				...
				found = FALSE;					/* Add v1 as a neighbour if it is _not_ already
												   a neighbour.								*/
				for (k = 0; k < V[i].vn; k++)
					if (v1 == V[i].VN[k])
						found = TRUE;
				...
				found = FALSE;					/* Add v2 as a neigbour if it is _not_ already
												   a neighbour.								*/
				for (k = 0; k < V[i].vn; k++)
					if (v2 == V[i].VN[k])
						found = TRUE;
				...
				if (ca < 0){					/* If this triangle is an obtuse angle		*/
					...
					if (count > mcount)			/* Update m count.							*/
						mcount = count;
												   ...
												   the stencil for this vertex _and_ triangle.*/
...
void FMM::CombineVectors(struct Vertex *(V[]), struct Triangle *(T[]), int *NonSplitTnum, 
	...
	*NonSplitTnum = Tnum / 3;
													...
													/* Allocate memory for both triangles _and_
												       vertixes.							*/
	...
	*V = (struct Vertex *)   malloc(sizeof(struct Vertex) * Vnum);
	...
	for(i = 0; i < Vnum; i++){						/* Move the data to V _and_ T.			*/
...
/* points specified by xplace _and_ yplace _and_ the index start_from that	   */
/* the algoritem gets. If the flag ALL_SOURCES_TOGETHER is defined the	   */
/* the procedure ignore the third argument _and_ use all points in xplace	   */
/* _and_ yplace as sources.												   */
/***************************************************************************/
...
FMM::update(struct Stencil *current_stencil, int k, int l){
	...
	double /*ctheta_mul_mb_div_ma_minus_one_mul_2,*/ ctheta_mul_ma_div_mb_minus_one_mul_2;
																			/* Both of them are between 0 _and_ 1 becuase
																			   the triangles are _not_ obuse.				*/
	...
	if (u >= 0) {
		...
		if (Quadratic(current_stencil->shortcut1_2,							/* If the quadratic equation has solutions */
				  ...
				  _and_ (-(u + u) >= ctheta_mul_ma_div_mb_minus_one_mul_2 * t)
				  _and_ current_stencil->Ctheta_mul_l1_div_l2_minus_one_mul_2*(t-u) <= (u + u))
				return (u1 + t);
		else									/* If the quadratic equation has no solution,
												   _or_ the solutions are out of the trinagle */
			return MAX_DISTANCE;
	}
	else{
		...
		if (Quadratic(current_stencil->shortcut1_1,						/* If the quadratic equation has solutions */
				  ...
				  _and_ (u + u >= ctheta_mul_ma_div_mb_minus_one_mul_2 * t)
				  _and_ current_stencil->Ctheta_mul_l2_div_l1_minus_one_mul_2*(t+u) <= -(u + u))
				return (u2 + t);
		else									/* If the quadratic equation has no solution,
												   _or_ the solutions are out of the trinagle */
			return MAX_DISTANCE;
...
FMM::TUpdateNeighbors(
	...
	for (st_ptr = V[i].ST, st_ind = V[i].si; st_ind > 0; st_ind--, st_ptr++) {
		...
		k = st_ptr->v1;
		l = st_ptr->v2;
		if(k == becomes_alive || l == becomes_alive){	/* Use this stencil to update only if one k _or_
...
//	mexPrintf("%d  (%d)\n", i,start_from);
	...
	for (VN_ind = V[i].VN, local_vn_count = 0; local_vn_count < V[i].vn; VN_ind++, local_vn_count++)
										/* If the neighbour is _not_ alive					*/
...
void FMM::MarchAlg(
	...
	InitMarch(srcval, Vnum);
									...
									/* CAllTUpdateNeighbours for every vertex when it becomes
									   ...
									   there are no more sources which are _not_ alive */
...
/* function with a surface matrix _and_ a sources matrix, the dll return the */
/* distances matrix _and_ can print various types of output, according to	   */
...
)
 {
    ...
    nrhs--;
  	...
  	if (!strcmpi(command, "init")) {
        ...
        ((uintptr_t *)mxGetPr(plhs[0]))[0] = reinterpret_cast<uintptr_t>(fmm);

    }
    else if (!strcmpi(command, "march")) {

	    FMM *fmm = (FMM*)((uintptr_t *)mxGetPr(prhs[0]))[0];
