function [boundary_old, boundary_new, boundary_edges_old] = findSegmentBoundary(Shape, segment)
% Finds the boundary of a segment of a shape.
...
numtri = size( Shape.surface.TRIV, 1);

all_edges = findEdgesTriangleList(Shape.surface, 1:numtri);
...
preBadTri = sum( ismember(Shape.surface.TRIV, boundary_old) , 2);
...
if ~isempty(BadTri)
    ...
    for i = 1:length(BadTri)
        ...
        for j = 1:3
            ...
            edge = [Shape.surface.TRIV(BadTri(i), j) Shape.surface.TRIV(BadTri(i), thingy(j + 1) )];
            [tri_indices, ~] = findTrianglesContainingEdge(Shape.surface, edge);
            
            num_tri_in_segment = 0;
            
            for k = 1:length(tri_indices)                
                tri = Shape.surface.TRIV(tri_indices(k),:);
