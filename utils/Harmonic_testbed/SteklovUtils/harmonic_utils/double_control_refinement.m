function double_control_refinement(obj,event_obj,S1,S2,X1,X2,B1,B2)
...
%             size(Basis2)
        ...
        [S1_opt, Basis1_opt] = refinementOptimization(S1, S2, Basis1, Basis2, landmarks1, landmarks2, NumControlLandmarks);
        
        pF_opt = annquery(Basis1_opt', Basis2', 1);
        pF_opt_2 = annquery( Basis2', Basis1_opt', 1);
        
        
        [S2_opt, Basis2_opt] = refinementOptimization(S2, S1, Basis2, Basis1_opt, landmarks2, landmarks1, NumControlLandmarks);
    ...
    subplot(3,3,4)    
    visualize_map_lines(S2.surface, S1_opt.surface, ...
        pF_opt, ...
        euclidean_fps(S2.surface,50));    
    title(sprintf('opt p2p map'));
    
    subplot(3,3,5)    
    visualize_map_lines(S1_opt.surface, S2.surface, ...
        pF_opt_2, ...
        euclidean_fps(S1_opt.surface,50));    
    title(sprintf('opt p2p map'));
    
    
    
    subplot(3,3,7)    
    visualize_map_lines(S2_opt.surface, S1_opt.surface, ...
        pF_opt_back, ...
        euclidean_fps(S2.surface,50));    
    title(sprintf('opt p2p map'));
    
    subplot(3,3,8)    
    visualize_map_lines(S1_opt.surface, S2_opt.surface, ...
        pF_opt_back_2, ...
        euclidean_fps(S1_opt.surface,50));    
    title(sprintf('opt p2p map'));
    ...
    subplot(3,3,6)
    trimesh(S1_opt.surface.TRIV, Basis1_opt(:,1), Basis1_opt(:,2), Basis1_opt(:,3), ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'k'); axis equal;  
    hold on;
    trimesh(S2.surface.TRIV, Basis2(:,1), Basis2(:,2), Basis2(:,3), ...
    ...
    trimesh(S1_opt.surface.TRIV, Basis1_opt(:,1), Basis1_opt(:,2), Basis1_opt(:,3), ...
        ...
        'FaceAlpha', 0.6, 'EdgeColor', 'k'); axis equal;  
    hold on;
    trimesh(S2_opt.surface.TRIV, Basis2_opt(:,1), Basis2_opt(:,2), Basis2_opt(:,3), ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'k'); axis equal; 
    ...
    subplot(1,4,3)
    trimesh(S1_opt.surface.TRIV, S1_opt.surface.X, S1_opt.surface.Y, S1_opt.surface.Z, ...
        opt_top_functions_1,...
        'FaceColor','interp', ...
        'FaceAlpha', 1, 'EdgeColor', 'none'); axis equal;    
    ...
    subplot(1,4,4)
    trimesh(S2_opt.surface.TRIV, S2_opt.surface.X, S2_opt.surface.Y, S2_opt.surface.Z, ...
    ...
    visualize_map_lines(S2_opt.surface, S1_opt.surface, ...
        pF_opt_back, ...
        euclidean_fps(S2_opt.surface,50),'k');    
    title(sprintf('opt back p2p map'));
    
    subplot(2,1,2)    
    visualize_map_lines(S1_opt.surface, S2_opt.surface, ...
        pF_opt_back_2, ...
        euclidean_fps(S1_opt.surface,50),'k');    
    title(sprintf('opt back p2p map'));
...
%         title('Both Opt_2 Embeddings')
    ...
    for p=1:numplots
        ...
        trimesh(S1_opt.surface.TRIV, Basis1_opt(:, funcombs(p,1)), Basis1_opt(:, funcombs(p,2)), Basis1_opt(:, funcombs(p,3)), ...
            'FaceColor','interp', ...
            'FaceAlpha', 0.6, 'EdgeColor', 'b'); axis equal;
        hold on;
        trimesh(S2_opt.surface.TRIV, Basis2_opt(:, funcombs(p,1)), Basis2_opt(:, funcombs(p,2)), Basis2_opt(:, funcombs(p,3)), ...
            'FaceColor','interp', ...
            'FaceAlpha', 0.6, 'EdgeColor', 'r'); axis equal;
%         title('Both Opt_2 Embeddings')
        xlabel(num2str(funcombs(p,1)))
    ...
    visualize_map_lines(S2_opt.surface, S1_opt.surface, ...
        pF_opt_back, ...
        euclidean_fps(S2_opt.surface,50));    
    title(sprintf('p2p map'));
...
%     title('Embedding S2')
    ...
    subplot(2,3,2)
    trimesh(S1_opt.surface.TRIV, Basis1_opt(:,1).^pow, Basis1_opt(:,2).^pow, Basis1_opt(:,3).^pow, ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'b'); axis equal;  
    hold on;
    trimesh(S2_opt.surface.TRIV, Basis2_opt(:,1).^pow, Basis2_opt(:,2).^pow, Basis2_opt(:,3).^pow, ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'r'); axis equal; 
    title('Both Embeddings')
...
%     [~, top_functions_2] = max(Basis2_opt);
    ...
    subplot(2,3,3)
    trimesh(S1_opt.surface.TRIV, Basis1_opt(:,4).^pow, Basis1_opt(:,5).^pow, Basis1_opt(:,6).^pow, ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'b'); axis equal;  
    hold on;
    trimesh(S2_opt.surface.TRIV, Basis2_opt(:,4).^pow, Basis2_opt(:,5).^pow, Basis2_opt(:,6).^pow, ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'r'); axis equal; 
    title('Both Embeddings')
...
%     [~, top_functions_2] = max(Basis2_opt);
    ...
    trimesh(S1_opt.surface.TRIV, S1_opt.surface.X, S1_opt.surface.Y, S1_opt.surface.Z, ...
        ...
        'FaceAlpha', 1, 'EdgeColor', 'none'); axis equal;
    hold on
    plot3(S1_opt.surface.VERT(landmarks1(1:end-NumControlLandmarks),1),S1_opt.surface.VERT(landmarks1(1:end-NumControlLandmarks),2),S1_opt.surface.VERT(landmarks1(1:end-NumControlLandmarks),3),'.','Color','r','MarkerSize',20)
    plot3(S1_opt.surface.VERT(landmarks1(end-NumControlLandmarks+1:end),1),S1_opt.surface.VERT(landmarks1(end-NumControlLandmarks+1:end),2),S1_opt.surface.VERT(landmarks1(end-NumControlLandmarks+1:end),3),'.','Color','b','MarkerSize',20)
    title('Regions S1')
    ...
    trimesh(S2_opt.surface.TRIV, S2_opt.surface.X, S2_opt.surface.Y, S2_opt.surface.Z, ...
        ...
        'FaceAlpha', 1, 'EdgeColor', 'none'); axis equal;
    hold on
    plot3(S2_opt.surface.VERT(landmarks2(1:end-NumControlLandmarks),1), S2_opt.surface.VERT(landmarks2(1:end-NumControlLandmarks),2), S2_opt.surface.VERT(landmarks2(1:end-NumControlLandmarks),3),'.','Color','r','MarkerSize',20)
    plot3(S2_opt.surface.VERT(landmarks2(end-NumControlLandmarks+1:end),1), S2_opt.surface.VERT(landmarks2(end-NumControlLandmarks+1:end),2), S2_opt.surface.VERT(landmarks2(end-NumControlLandmarks+1:end),3),'.','Color','b','MarkerSize',20)
    title('Regions S2')
