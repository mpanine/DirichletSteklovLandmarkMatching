function eF = extendSegmentFunctionsIntegrals_principled(F, boundaries_segment, Mass_Boundary, segment, landmarks, nv)
...
% boundaries_segment: list of boundaries of segment
...
if length(size(F))==2
    ...
    for i = 1:length(landmarks)
        ...
        eF(landmarks(i),:) = sum(Mass_Boundary{i}*F(boundaries_segment{i},:),1);% / sum(sum( Mass_Boundary{i} )); % Values at the landmarks are integrals over the boundary.
...
elseif length(size(F))==3
    ...
    for i = 1:length(landmarks) %Cycle over the boundaries with nonzero integrals
        ...
        eF(landmarks(i),:,i) = sum(Mass_Boundary{i}*F(boundaries_segment{i},:,i),1);% / sum(sum( Mass_Boundary{i} )); % Values at the landmarks are integrals over the boundary.
