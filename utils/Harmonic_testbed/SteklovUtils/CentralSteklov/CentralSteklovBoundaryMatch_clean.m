function [FmapSrcTar, p2pTarSrc, FmapTarSrc, p2pSrcTar] = CentralSteklovBoundaryMatch_clean(Src_refined, Tar_refined, Steklov_settings)
%CentralSteklovBoundaryMatch_clean: Matches the boundary circles by
...
[~, Src_ND_expand] = CentralSteklovKroneckerFunctions(Src_refined);
[~, Tar_ND_expand] = CentralSteklovKroneckerFunctions(Tar_refined);
...
for b = 1:num_bounds % Iterate over each boundary
    ...
    for i = 1:(num_bounds-1)        
        ...
        Src_ND(:,i) = Src_refined.STEKLOV.evecs_bound{b} * Src_ND_expand(:,inds(i));
        Tar_ND(:,i) = Tar_refined.STEKLOV.evecs_bound{b} * Tar_ND_expand(:,inds(i));        
...
%     FmapTarSrc = CentralSteklovFmapFromP2P(Tar_refined.STEKLOV.evecs_bound, Src_refined.STEKLOV.evecs_bound, Src_refined.STEKLOV.MassBoundary, p2pSrcTar, num_eigs);
    ...
    FmapSrcTar = CentralSteklovFmapFromP2P(Src_refined.STEKLOV.evecs_bound, Tar_refined.STEKLOV.evecs_bound, Tar_refined.STEKLOV.MassBoundary, p2pTarSrc, it);
    FmapTarSrc = CentralSteklovFmapFromP2P(Tar_refined.STEKLOV.evecs_bound, Src_refined.STEKLOV.evecs_bound, Src_refined.STEKLOV.MassBoundary, p2pSrcTar, it);
    
    
%     p2pTarSrc = DirichletSteklovFmapToP2P(Src_evecs_bound, Src_evals_mod, Tar_evecs_bound, Tar_evals_mod, FmapSrcTar, it);    
    
    [p2pTarSrc, p2pSrcTar] = CentralSteklovFmapToP2P_clean(Src_refined.STEKLOV.evecs_bound, Src_refined.STEKLOV.evals,...
