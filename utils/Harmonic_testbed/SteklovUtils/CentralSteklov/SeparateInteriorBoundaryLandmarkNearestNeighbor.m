function p2p = SeparateInteriorBoundaryLandmarkNearestNeighbor(S1_refined, S1_F, S1_landmarks, S2_refined, S2_F, S2_landmarks)
%function p2p = SeparateInteriorBoundaryLandmarkNearestNeighbor(S1_refined, S1_F, S2_refined, S2_F)
%A nearest neighbor search on functions that explicitly maps the 
%landmarks to the landmarks, the landmark boundaries to the corresponding landmark boundaries
...
for i = 1:length(S1_refined.STEKLOV.boundaries) % On the boundaries
   ...
   p2p(S2_refined.STEKLOV.boundaries{i}) = S1_refined.STEKLOV.boundaries{i}(...
                                                        annquery(S1_F(S1_refined.STEKLOV.boundaries{i}, :)',...
                                                        S2_F(S2_refined.STEKLOV.boundaries{i}, :)', 1)....
                                                    );  
