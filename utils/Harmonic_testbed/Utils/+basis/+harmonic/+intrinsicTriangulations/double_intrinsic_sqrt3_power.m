function double_intrinsic_sqrt3_power(obj,event_obj,S1,S2,X1,X2,B1,B2)
...
%         [S1, Basis1_opt] = refinementOptimization(S1, S2, Basis1, Basis2, landmarks1, landmarks2, NumControlLandmarks);
        ...
        powers = ones( length(landmarks1) - NumControlLandmarks , 1);
...
%         [InitialDistance, InitGrad] = controlLandmarkDistance(Basis2, Basis1_opt, landmarks2, landmarks1, NumControlLandmarks, powers);
%         InitialDistance
%         InitGrad;
        
        funObj = @(v) controlLandmarkDistance(Basis2_opt, Basis1_opt, landmarks2, landmarks1, NumControlLandmarks, v);
        funProj = @(F) F;
        ...
        Basis1_power_opt = Basis1_opt.^powers;
        
        pF_opt = annquery( Basis1_power_opt', Basis2_opt', 1);
        pF_opt_2 = annquery( Basis2_opt', Basis1_power_opt', 1);

        
        
        
        powers_back = ones( length(landmarks1) - NumControlLandmarks , 1);
%         [InitialDistance, InitGrad] = controlLandmarkDistance(Basis1_opt.^powers, Basis2_opt, landmarks1, landmarks2, NumControlLandmarks, powers_back);
%         InitialDistance
%         InitGrad;
        
%         funObj = @(v) controlLandmarkDistance(Basis1_power_opt, Basis2_opt, landmarks1, landmarks2, NumControlLandmarks, v);
        funObj = @(v) controlLandmarkDistance(Basis1_power_opt, Basis2_opt, landmarks1, landmarks2, NumControlLandmarks, v);
        funProj = @(F) F;
        
        powers_back = minConf_PQN( funObj, powers_back, funProj, options);
        powers_back = powers_back';
        
        Basis2_power_opt = Basis2_opt.^powers_back; 
        
        
        pF_opt_back = annquery( Basis1_power_opt', Basis2_power_opt', 1);
        pF_opt_back_2 = annquery( Basis2_power_opt', Basis1_power_opt', 1);
    ...
    subplot(3,3,6)
    trimesh(S1.surface.TRIV, Basis1_power_opt(:,1), Basis1_power_opt(:,2), Basis1_power_opt(:,3), ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'k'); axis equal;  
    hold on;
    trimesh(S2.surface.TRIV, Basis2(:,1), Basis2(:,2), Basis2(:,3), ...
    ...
    trimesh(S1.surface.TRIV, Basis1_power_opt(:,1), Basis1_power_opt(:,2), Basis1_power_opt(:,3), ...
        ...
        'FaceAlpha', 0.6, 'EdgeColor', 'k'); axis equal;  
    hold on;
    trimesh(S2.surface.TRIV, Basis2_power_opt(:,1), Basis2_power_opt(:,2), Basis2_power_opt(:,3), ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'k'); axis equal; 
    ...
    [~, opt_top_functions_1] = max(Basis1_power_opt');
    
    [~, back_opt_top_functions_2] = max(Basis2_power_opt');
...
%         title('Both Opt_2 Embeddings')
    ...
    for p=1:min(numplots,64)
        ...
        trimesh(S1.surface.TRIV, Basis1_power_opt(:, funcombs(p,1)), Basis1_power_opt(:, funcombs(p,2)), Basis1_power_opt(:, funcombs(p,3)), ...
            'FaceColor','interp', ...
            'FaceAlpha', 0.6, 'EdgeColor', 'b'); axis equal;
        hold on;
        trimesh(S2.surface.TRIV, Basis2_power_opt(:, funcombs(p,1)), Basis2_power_opt(:, funcombs(p,2)), Basis2_power_opt(:, funcombs(p,3)), ...
            'FaceColor','interp', ...
            'FaceAlpha', 0.6, 'EdgeColor', 'r'); axis equal;
%         title('Both Opt_2 Embeddings')
        xlabel(num2str(funcombs(p,1)))
...
%     title('Embedding S2')
    ...
    subplot(2,3,2)
    trimesh(S1.surface.TRIV, Basis1_power_opt(:,1).^pow, Basis1_power_opt(:,2).^pow, Basis1_power_opt(:,3).^pow, ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'b'); axis equal;  
    hold on;
    trimesh(S2.surface.TRIV, Basis2_power_opt(:,1).^pow, Basis2_power_opt(:,2).^pow, Basis2_power_opt(:,3).^pow, ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'r'); axis equal; 
    title('Both Embeddings')
...
%     [~, top_functions_2] = max(Basis2_opt);
    ...
    subplot(2,3,3)
    trimesh(S1.surface.TRIV, Basis1_power_opt(:,4).^pow, Basis1_power_opt(:,5).^pow, Basis1_power_opt(:,6).^pow, ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'b'); axis equal;  
    hold on;
    trimesh(S2.surface.TRIV, Basis2_power_opt(:,4).^pow, Basis2_power_opt(:,5).^pow, Basis2_power_opt(:,6).^pow, ...
        'FaceColor','interp', ...
        'FaceAlpha', 0.6, 'EdgeColor', 'r'); axis equal; 
    title('Both Embeddings')
