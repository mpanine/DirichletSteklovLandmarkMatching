function [harmonic_basis_1_opt, current_radf] = intrinsicOptimizationNaive(shape_number, S1, S2, harmonic_basis_1, harmonic_basis_2, landmarks1, landmarks2, NumControlLandmarks, radii1)
...
%         dist = Inf
        ...
        radf = current_radf;
        radf(l) = radf(l) + stepsign*0.000001; % First we modify the radius according to stepsign

%         if radf(l) > maxradf
%            radf(l) = maxradf; 
%         end

%         S1_opt_t = insertCentralFElist(S1_opt, landmarks1(1:end-NumControlLandmarks), radf.*radii1);
%         harmonic_basis_1_opt_t = ComputeCentralHarmonicBasis(S1_opt_t, landmarks1(1:end-NumControlLandmarks));
        harmonic_basis_1_opt_t = computeIntrinsicBasis( shape_number, S1.nv, landmarks1(1:end-NumControlLandmarks), radf );
...
%             S1_opt = S1_opt_t;
            harmonic_basis_1_opt = harmonic_basis_1_opt_t;
                ...
                radf = current_radf;
                radf(l) = radf(l) + stepsign*0.000001;
