...
default_param.checkFaceColor = @(x) (isnumeric(x)&&length(x)==3)||any(validatestring(x,validFaceColor));

default_param.EdgeColor = 'none';
validEdgeColor = {'none','flat','interp'};
default_param.checkEdgeColor = @(x) (isnumeric(x)&&length(x)==3)||any(validatestring(x,validEdgeColor));
